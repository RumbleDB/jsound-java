...
public class InvalidDefaultTest extends BaseTest {
    ...
    public void initializeApplication() throws IOException {
        jSoundSchema = JSoundExecutor.loadSchemaFromPath(
                schemaPathPrefix + "extendedSchemas/object/invalidDefaultSchema.json",
                "targetType",
                false
        );
    }
    ...
    public void validateDefaultValues() {
        for (FieldDescriptor fieldDescriptor : schema.get("objectObj").getFacets().getObjectContent().values()) {
            assertFalse(
