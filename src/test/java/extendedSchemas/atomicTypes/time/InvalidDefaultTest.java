...
public class InvalidDefaultTest extends BaseTest {
    ...
    public void initializeApplication() throws IOException {
        jSoundSchema = JSoundExecutor.loadSchemaFromPath(
                schemaPathPrefix + "extendedSchemas/atomicTypes/time/invalidDefaultSchema.json",
                "targetType",
                false
        );
    }
    ...
    public void validateDefaultValues() {
        for (FieldDescriptor fieldDescriptor : schema.get("timeObj").getFacets().getObjectContent().values()) {
            assertFalse(
